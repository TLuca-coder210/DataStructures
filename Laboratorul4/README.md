...
#### Space: $\mathcal{O}(n)$ total storage to maintain the vector-backed nodes, using $\mathcal{O}(1)$ auxiliary space for iterative operations and $\mathcal{O}(\ log  \n)$ recursion stack during initial construction.
